...
class ModelWrite:
    ...
    def registered_name(self) -> str | None:
        ...
        A create registers its ``model_name``; a rename registers the new one; a config-only
        update or a delete registers nothing, so a name that became a collision after the router
        was created never locks its creator out of editing or removing it.
        """
        incoming_name: Final = None if self.incoming is None else self.incoming.model_name
        if incoming_name is not None:
            return incoming_name
        return None if self.stored is not None or self.incoming is None else self.incoming.model_name
...
def member_may_write_auto_router(
    ...
    The member must hold the team grant; the row must be a strategy router before and after the
    write (so neither a router nor a regular model can be converted), configured inline rather than
    from a file on the proxy host; an existing row must be one the member created; and a public name
    the write registers must not widen the team's model access.
    """
    if not team_member_has_permission(
